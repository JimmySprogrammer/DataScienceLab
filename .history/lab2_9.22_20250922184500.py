...
try:
    ...
    if 'SalePrice' in df.columns:
        ...
        print(f"⚠️ 未找到标准的目标变量名，使用 '{target_col}' 作为目标变量")
...
if len(missing_data) > 0:
    ...
    if len(high_missing_columns) > 0:
        ...
        print(f"🗑️ 已删除缺失率 > {high_missing_threshold}% 的特征: {list(high_missing_columns)}")
    else:
...
if 'GrLivArea' in df_cleaned.columns and target_col in df_cleaned.columns:
    ...
    print(f"🗑️ 删除了 {removed_count} 个明显异常点")
...
if target_col in num_columns_final:
    ...
    print(f"⚠️ 目标变量 {target_col} 不是数值类型，无法进行相关性分析")
...
if target_col in num_columns_final:
    ...
    for i, feature in enumerate(top_3_features, 1):
        if feature in feature_explanations:
            ...
            print(f"\n{i}. {feature}:")
            print("   ℹ️ 这是与房价高度相关的数值特征，具体含义需要参考数据描述文档")
